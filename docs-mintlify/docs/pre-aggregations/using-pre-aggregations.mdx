...
description: Operational guide to aggregate awareness—how Cube picks rollups, partitions data, enforces rollup-only mode, and tunes pre-aggregation behavior.
---
...
[`CUBEJS_ROLLUP_ONLY`](/reference/configuration/environment-variables#cubejs_rollup_only) environment variable.
...
[ref-deploy-refresh-wrkr]: /docs/deployment#refresh-worker
[ref-prod-list-refresh]: /docs/deployment/production-checklist#set-up-refresh-worker
...
[ref-additivity]: /docs/pre-aggregations/getting-started-pre-aggregations#additivity
